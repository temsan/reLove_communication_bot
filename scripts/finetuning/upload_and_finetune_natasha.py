#!/usr/bin/env python3
"""
Загрузка датасета и запуск fine-tuning модели Наташи через OpenAI API.

Следует документации: https://platform.openai.com/docs/guides/supervised-fine-tuning
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/finetune_natasha.log', encoding='utf-8'),
...
class NatashaFineTuner:
    """Класс для fine-tuning модели Наташи."""
    ...
    def list_jobs(self, limit: int = 10) -> list:
        ...
        logger.info(f"\nListing last {limit} fine-tuning jobs...")
    ...
    def save_config(self, output_path: str = "data/finetune_config.json"):
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload dataset and run fine-tuning for Natasha model"
    )
    parser.add_argument(
        '--file',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default='gpt-4-turbo',
        help='Base model for fine-tuning (default: gpt-4-turbo)'
    )
    parser.add_argument(
        '--epochs',
        type=int,
    ...
    print("\n" + "="*70)
    print("NATASHA FINE-TUNING UPLOADER")
    print("="*70 + "\n")
    
    tuner = NatashaFineTuner()
    ...
    try:
        ...
        if args.test:
            ...
            test_results_file = f"data/test_results_{args.test}.json"
        ...
        if success:
            ...
            # Сохраняем результаты
            test_results_file = f"data/test_results_{tuner.model_id}.json"
            with open(test_results_file, 'w', encoding='utf-8') as f:
                json.dump(results, f, ensure_ascii=False, indent=2)
            logger.info(f"✅ Test results saved to: {test_results_file}")
